...
function parsed(line) {
  try { return new URL(line); } catch { return null; }
}

function hostOf(line) {
  return parsed(line)?.searchParams.get('host') || '';
}

function finalMaskOf(line) {
  const value = parsed(line)?.searchParams.get('fm');
  if (!value) return null;
  try { return JSON.parse(decodeURIComponent(value)); } catch { return null; }
}

function isExpectedFinalMask(mask) {
  const fragment = mask?.tcp?.[0];
  const settings = fragment?.settings;
  return mask?.tcp?.length === 1 &&
    fragment?.type === 'fragment' &&
    String(settings?.packets) === 'tlshello' &&
    String(settings?.length) === '2' &&
    String(settings?.delay) === '0-1' &&
    String(settings?.maxSplit) === '3-6';
}
...
const backupPath = path.join(backupRoot, `cdn-finalmask-only-${timestamp.replace(/[:.]/g, '-')}.json`);
...
try {
  ...
  for (const user of users) {
    ...
    for (const line of after.filter((item) => targetHosts.has(hostOf(item)))) {
      const url = parsed(line);
      const mask = finalMaskOf(line);
      if (!isExpectedFinalMask(mask)) {
        throw new Error(`${user.id}: invalid FinalMask for ${hostOf(line)}`);
      }
      if (url.searchParams.has('fragment') || url.searchParams.has('mux')) {
        throw new Error(`${user.id}: conflicting CDN parameters for ${hostOf(line)}`);
      }
