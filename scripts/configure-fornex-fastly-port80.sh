...
UNIT=fornex-fastly-port80-redirect.service
XRAY_UNIT=xray-fornex-test.service
...
if ss -ltnH | awk '{print $4}' | grep -q ":${PUBLIC_PORT}$"; then
  echo "Fornex public port ${PUBLIC_PORT} is already occupied" >&2
  exit 1
fi

cat > "/etc/systemd/system/$UNIT" <<EOF
[Unit]
Description=Redirect Fastly HTTP origin port 80 to Fornex xHTTP
...
echo "FORNEX_FASTLY_PORT80_OK xrayPid=$XRAY_PID_AFTER"
