...
const backupPath = path.join(backupRoot, `happ-android-compat-${user.id}-${timestamp.replace(/[:.]/g, '-')}.json`);
await writeFile(backupPath, JSON.stringify({
  createdAt: timestamp,
  userId: user.id,
  previous: user.happAndroidCompatibility ?? null,
}, null, 2), 'utf8');
await updateUser(user.id, { happAndroidCompatibility: true, updatedAt: timestamp });
await upsertUserSubscriptionFile({ ...user, happAndroidCompatibility: true, updatedAt: timestamp });
console.log(JSON.stringify({
  ok: true,
  user: { id: user.id, name: user.name },
  happAndroidCompatibility: true,
  backupPath,
}, null, 2));
