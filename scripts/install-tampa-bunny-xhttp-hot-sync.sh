#!/bin/bash
# Tampa runs sync agents in Docker (host Node is v12). Mirror bunny-tampa-sync.
set -euo pipefail

EDGE_ID=bunny-xhttp-tampa
CONFIG=/opt/vpn-dayanch-bunny-xhttp/config.json
UNIT=xray-dayanch-bunny-xhttp.service
INBOUND_TAG=dayanch-bunny-xhttp-in
API_PORT=10098
AGENT_PORT=19233
XRAY_BIN=/opt/vpn-dayanch-bunny-xhttp/xray-26.3.27
SOURCE_DIR=/opt/vpn-standalone-sync-bunny-tampa
TARGET_DIR=/opt/vpn-standalone-sync-bunny-xhttp-tampa
CONTAINER_NAME=vpn-bunny-xhttp-tampa-sync
EDGE_DIR=/opt/vpn-dayanch-bunny-xhttp
...
# Stop broken host systemd unit if present
systemctl disable --now vpn-standalone-sync-bunny-xhttp-tampa.service 2>/dev/null || true

# Patch API into xray config (same as other origins)
...
"$XRAY_BIN" run -test -config "$CONFIG" >/tmp/tampa-xhttp-api-test.log 2>&1
systemctl restart "$UNIT"
sleep 2
systemctl is-active --quiet "$UNIT"

# Clone docker-based agent tree
rm -rf "$TARGET_DIR"
...
# Ensure compose mounts the custom xray binary path
...
echo "BUNNY_XHTTP_HOT_SYNC_OK edge=${EDGE_ID} api=${API_PORT} agent=${AGENT_PORT} mode=docker"
