#!/usr/bin/env python3
"""Persist UDP/443 blocking while the running Xray is updated through its API."""
...
def main() -> int:
    ...
    for outbound in config.get("outbounds", []):
        ...
        if outbound.get("protocol") != "blackhole":
            outbound.clear()
            outbound.update({"tag": "block", "protocol": "blackhole", "settings": {}})
            changed += 1
    ...
    backup = f"{path}.pre-quic-disable-{stamp}.bak"
